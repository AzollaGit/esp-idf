...
// cd examples/bluetooth/bluedroid/ble/blufi/
...
static bool sta_got_ip_event = false;   // 1: 连上wifi
...
// 如果已经连上wifi了就不调用；以免开启蓝牙浪费资源；等需要重新配网时再开启！
...
void app_blufi_init()
{
    ...
    uint8_t bulfi_init_timeout = 0;
    while (bulfi_init_timeout < 10) {
